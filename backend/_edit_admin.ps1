# PowerShell script to add Penghasilan (Revenue) tab to admin.php
$file = "website/backend/admin.php"
$content = Get-Content $file -Raw

# 1. Add tab button after Push ke Git button
$oldTab = @"
...
# 2. Update panels array in showPanel function
                ...
                <table class="w-full text-left text-sm">
                    <thead class="bg-slate-50 text-slate-600 font-semibold border-b border-slate-200">
                        <tr>
                            ...
                            <th class="px-5 py-3 text-right">Jumlah Transaksi</th>
...
if ($content.Contains($closingMain)) {
    $content = $content.Replace($closingMain, $panelPenghasilan + $closingMain)
    Write-Host "Panel HTML added successfully"
} else {
    Write-Host "ERROR: Could not find closing main tag"
    exit 1
}
...
function setRevRange(range) {
    ...
    let mulai, selesai = tglStr;
    if (range === 'today') {
        mulai = tglStr;
    } else if (range === 'week') {
        const weekAgo = new Date(today);
        weekAgo.setDate(weekAgo.getDate() - 6);
        mulai = weekAgo.getFullYear() + '-' + String(weekAgo.getMonth() + 1).padStart(2, '0') + '-' + String(weekAgo.getDate()).padStart(2, '0');
    } else if (range === 'month') {
        mulai = y + '-' + m + '-01';
    } else if (range === 'year') {
        mulai = y + '-01-01';
    ...
    document.getElementById('rev-tgl-selesai').value = selesai;
    loadRevenueData();
}
...
function loadRevenueData() {
    ...
    if (!mulai || !selesai) {
        showNotification('Pilih tanggal mulai dan selesai terlebih dahulu.', 'error');
        return;
    }

    const loading = document.getElementById('rev-loading');
    const summary = document.getElementById('rev-summary');
    const daily = document.getElementById('rev-daily-section');
    const trans = document.getElementById('rev-trans-section');
    const empty = document.getElementById('rev-empty');
    const error = document.getElementById('rev-error');
    const btn = document.getElementById('btn-rev-load');
    ...
    btn.disabled = true;
    btn.innerHTML = '<i class="fa-solid fa-spinner animate-spin"></i> Memuat...';
    ...
    fetch('api_penghasilan.php', { method: 'POST', body: fd })
        ...
        .then(data => {
            ...
            if (!data.success) {
                ...
                document.getElementById('rev-error-text').textContent = data.message;
                return;
            }

            const d = data.data;

            if (d.total_transaksi === 0) {
            ...
            // Daily breakdown
            if (d.harian && d.harian.length > 0) {
                renderRevDaily(d.harian);
                document.getElementById('rev-daily-count').textContent = d.harian.length + ' hari';
                daily.classList.remove('hidden');
            }

            // Recent transactions
            if (d.transaksi_terbaru && d.transaksi_terbaru.length > 0) {
                renderRevTransactions(d.transaksi_terbaru);
                document.getElementById('rev-trans-count').textContent = d.transaksi_terbaru.length + ' transaksi';
        ...
        .catch(err => {
            ...
            btn.innerHTML = '<i class="fa-solid fa-magnifying-glass"></i> Tampilkan';
            error.classList.remove('hidden');
            document.getElementById('rev-error-text').textContent = 'Gagal terhubung ke server.';
        });
}

function renderRevSummary(d) {
    const container = document.getElementById('rev-summary');
    ...
    // Add profit card if available
    if (d.profit) {
        cards.push({
            icon: 'fa-coins',
            color: 'text-green-600 bg-green-50 border-green-200',
            label: 'Laba Kotor',
            value: fmt(d.profit.total_laba_kotor)
        });
        cards.push({
            icon: 'fa-cube',
            color: 'text-sky-600 bg-sky-50 border-sky-200',
            label: 'Item Terjual',
            value: d.profit.total_item_terjual + ' item'
        });
        cards.push({
            icon: 'fa-cart-shopping',
            color: 'text-slate-600 bg-slate-50 border-slate-200',
            label: 'Total Modal',
            value: fmt(d.profit.total_modal)
        });
    }
    ...
    container.innerHTML = cards.map(c => `
        <div class="bg-white rounded-xl border ${c.color.split(' ')[2]} shadow-sm p-5">
            <div class="flex items-center gap-3 mb-3">
                <div class="w-10 h-10 rounded-lg ${c.color.split(' ').slice(1,3).join(' ')} flex items-center justify-center">
                    <i class="fa-solid ${c.icon} ${c.color.split(' ')[0]}"></i>
                </div>
...
function renderRevDaily(harian) {
    const tbody = document.getElementById('rev-daily-body');
    const fmt = (v) => 'Rp ' + Number(v).toLocaleString('id-ID');
    tbody.innerHTML = harian.map(h => {
        const tgl = new Date(h.tgl + 'T00:00:00');
        const tglStr = tgl.toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
        return '<tr class="hover:bg-slate-50 transition-colors">' +
            '<td class="px-5 py-3 font-semibold text-slate-700">' + tglStr + '</td>' +
            '<td class="px-5 py-3 text-right font-bold text-slate-600">' + h.jumlah + '</td>' +
            '<td class="px-5 py-3 text-right font-bold text-emerald-600">' + fmt(h.total) + '</td>' +
        '</tr>';
    }).join('');
}

function renderRevTransactions(transaksi) {
    const tbody = document.getElementById('rev-trans-body');
    const fmt = (v) => 'Rp ' + Number(v).toLocaleString('id-ID');
    tbody.innerHTML = transaksi.map(t => {
        const tgl = new Date(t.tgl + 'T00:00:00');
        const tglStr = tgl.toLocaleDateString('id-ID', { year: 'numeric', month: 'short', day: 'numeric' });
        return '<tr class="hover:bg-slate-50 transition-colors">' +
            '<td class="px-5 py-3 font-mono text-xs font-bold text-astra-700">' + escHtml(t.notransaksi) + '</td>' +
            '<td class="px-5 py-3 text-sm text-slate-600">' + tglStr + '</td>' +
            '<td class="px-5 py-3 text-sm text-slate-600">' + escHtml(t.pelanggan) + '</td>' +
            '<td class="px-5 py-3 text-right font-bold text-emerald-600">' + fmt(t.totalakhir) + '</td>' +
        '</tr>';
    }).join('');
}
"@
...
if ($content.Contains($scriptEnd)) {
    $content = $content.Replace($scriptEnd, $revFunctions + "`r`n" + $scriptEnd)

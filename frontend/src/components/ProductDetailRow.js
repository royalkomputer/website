 ...
 *
 * Renders a single product as a horizontal row with optional image,
 * product name, category, condition badge, price, and WhatsApp button.
 *
 * @param {Object} product — product object from the API
 * @param {Object} [options]
 * @param {boolean} [options.hideImage] — if true, render text-only without image
 * @returns {string} HTML string
 */
export function ProductDetailRow(product, { hideImage = false } = {}) {
...
const imageHtml = hideImage ? '' : `
  <div class="w-24 sm:w-28 md:w-32 flex-shrink-0 bg-slate-800 overflow-hidden aspect-[4/3]">
    <img src="${product.image}" alt="${product.name}" loading="lazy" onerror="this.src='${fallbackSvg}'" width="200" height="150"
  ...
  <div class="flex-1 flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3 p-3 sm:p-4 min-w-0">
    <div class="flex flex-col gap-1 min-w-0 flex-1 w-full sm:w-auto">
      <div class="flex items-center gap-2 flex-wrap">
        <h3 class="font-bold text-white text-sm sm:text-base leading-tight line-clamp-1">${product.name}</h3>
        <span class="hidden sm:inline-flex">${badgeKondisi}</span>
      </div>
      <div class="flex items-center gap-2 sm:hidden flex-wrap">
        ${badgeKondisi}
        <span class="bg-astra-900/40 text-astra-300 text-[9px] font-bold px-2 py-0.5 rounded border border-astra-800">${product.category}</span>
      </div>
